...
echo "Preparing logfiles finished" | tee search-index-typesense/logs/succes.log

# Create sitemaps.
source "$SCRIPT_DIR/createSitemaps.sh"
echo "Creating sitemaps finished" | tee search-index-typesense/logs/succes.log

# Remove unwanted urls from the sitemaps (new sitemaps generated or not)
node "$SCRIPT_DIR/removeURLsFromSitemap.mjs"
echo "Extracting data finished" | tee search-index-typesense/logs/succes.log

# Filenames to lowercase.
node "$SCRIPT_DIR/renameFilesToLowerCase.mjs" search-index-typesense/sitemaps
echo "Renaming files to lowercase finished" | tee search-index-typesense/logs/succes.log
...
echo "Sorting and styling index file finished" | tee search-index-typesense/logs/succes.log

# Make collection in Typesense empty.
source "$SCRIPT_DIR/make-collection-empty.sh"
echo "Making collection empty finished" | tee search-index-typesense/logs/succes.log

# Import the data into Typesense.
source "$SCRIPT_DIR/import.sh"
echo "Importing data finished" | tee search-index-typesense/logs/succes.log

# Finding ids to urls.
source "$SCRIPT_DIR/urlToID.sh"
echo "Finding ids to urls finished" | tee search-index-typesense/logs/succes.log

# Import overrides into Typesense.
source "$SCRIPT_DIR/overrides.sh"
echo "Importing overrides finished" | tee search-index-typesense/logs/succes.log